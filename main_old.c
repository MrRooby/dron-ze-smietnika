...
int main(void){
  ...
  while(true){
    ...
    // If we received valid rotor control data, apply it
    if (pwm_data->valid) {
      TIM1_SetCompare3(pwm_data->O1);  // Front rotor
      TIM1_SetCompare4(pwm_data->O2);  // Back rotor
      TIM2_SetCompare2(pwm_data->B1);  // Left rotor
      TIM2_SetCompare3(pwm_data->B2);  // Right rotor
